...
class CategoryController extends Controller{
    ...
    public function  getStore($id){
        $store = new Store();
        $data = $store->selectForigen('category_id',$id)->get();
        $dataStore = [];

        foreach ($data as $item){
            $stores = new Store();
            $stores->setId($item['id']);
            $stores->setName($item['name']);
            $stores->setDescription($item['description']);
            $stores->setLogo($item['logo']);
            array_push($dataStore,$stores);
        }
